...
class StageLinQService:
    def __init__(self, ip, discovery_frame, own_token, service_found_callback):
        ...
        self.Token = own_token
        self.DeviceToken = PyStageLinQToken.StageLinQToken()
        self.service_list = []
    ...
    def get_services(self):
        if self.services_available:
            return [self.device_name, self.Ip, self.service_list]
    ...
    async def send_request_frame(self, delay=0):
        ...
        service_request_message = out_data.encode(StageLinQServiceRequestService(Token=self.Token))
    ...
    async def send_reference_message(self):
        ...
        if self.services_available:
            reference_data = StageLinQReferenceData(OwnToken=self.Token, DeviceToken=self.DeviceToken, Reference=0)
